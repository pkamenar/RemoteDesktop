$RemoteDesktop = Get-Package -Provider Programs -IncludeWindowsInstaller -Name "Remote Desktop" | select name
if(-not $RemoteDesktop) {
Write-Host 'Remote Desktop version is not installed, lates version is going to be downloaded and installed.' -BackgroundColor Red -ForegroundColor Yellow 
}
else {
Write-Host 'Uninstalling current Remote Desktop installation, please wait.' -BackgroundColor Green -ForegroundColor Black
Uninstall-Package $RemoteDesktop.Name
