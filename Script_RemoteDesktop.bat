$RemoteDesktop = Get-WmiObject -Class Win32_Product | Where-Object{$_.Name -eq "Remote Desktop"}
$RemoteDesktop
$isInstalled = (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | where { $_.DisplayName -eq "Remote Desktop" }) -ne $null
if(-not $isInstalled) {
Write-Host "Remote Desktop is not installed, application is going to be downloaded and installed." -BackgroundColor Red -ForegroundColor Yellow 
}
else {
...
if (test-Path -Path $checkTempFolder) {
    ...
    Write-host "Temp folder doesnt't exist!" -BackgroundColor Red -ForegroundColor Yellow 
